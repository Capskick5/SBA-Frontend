...
export default function AdminUsersPage() {
  ...
  return (
    <section className="stack">
      ...
      {loading ? (
        <LoadingState text="Đang tải người dùng..." />
      ) : error ? (
        <ErrorState text={error}>
          <Button onClick={() => fetchUsers(currentPage)}>Thử lại</Button>
        </ErrorState>
      ) : (
        <>
          <Table
            ...
            columns={[
              ...
              },
              {
                key: 'enabled',
                label: 'Trạng thái',
                render: (row) => (row.enabled ? 'Đang hoạt động' : 'Đã khóa'),
              },
              {
                key: 'action',
                label: 'Thao tác',
                  ...
                  row.role === 'ADMIN' ? (
                    ...
                    <div className="admin-row-actions">
                      <Button
                        ...
                        variant="secondary"
                        size="sm"
                        className={row.enabled ? 'danger-action' : ''}
