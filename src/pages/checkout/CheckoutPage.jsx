...
export default function CheckoutPage() {
  ...
  const refreshGuestPreview = async (
    address,
    itemIds,
    fallbackItems = selectedCartItems,
    mode = deliveryMode,
  ) => {
    if (!isAddressComplete(address) || itemIds.length === 0) {
    ...
    try {
      const previewData = await checkoutService.guestPreview(
        address,
        itemIds,
      ...
      // Guest APIs may not be live yet — keep local merchandise totals.
      setPreview(buildCartPreview(fallbackItems));
    }
  };
    ...
    Promise.resolve().then(async () => {
      ...
      if (isGuest) {
        ...
        if (guestAddress && itemIds.length > 0) {
          await refreshGuestPreview(guestAddress, itemIds, selectedItems);
        }
        return;
      }
  ...
  const handleDeliveryModeChange = async (mode) => {
    ...
    try {
      if (isGuest) {
        if (!isAddressComplete(guestAddress)) return;
        await refreshGuestPreview(guestAddress, cartItemIds, selectedCartItems, mode);
        return;
      }
  ...
  const handleGuestAddressSubmit = async (payload) => {
    setFormLoading(true);
    setFormError(null);
    setFormFieldErrors({});
    setCheckoutError('');
    try {
      ...
      await refreshGuestPreview(address, cartItemIds, selectedCartItems);
    } catch (err) {
  ...
  const pay = async () => {
    ...
    try {
      ...
      const guestApiMissing = isGuest && (
        ...
        || err?.error_type === 'RESOURCE_NOT_FOUND'
      );
      if (guestApiMissing) {
        setCheckoutError('Guest checkout API is not available yet. Please sign in to complete your order, or try again after the backend update.');
      } else {
